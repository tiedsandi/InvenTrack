...
export default function Create({ suppliers, products }) {
    ...
    const submit = (e) => { e.preventDefault(); post('/purchase-orders'); };

    return (
        <AppLayout title="Transaksi — Buat Purchase Order">
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                        <div className="px-5 py-4 border-b border-slate-100">
                            <h6 className="font-semibold text-slate-700">Informasi Purchase Order</h6>
                        </div>
                        <div className="p-5 grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                <label className="block text-sm font-semibold text-slate-700 mb-1">Supplier <span className="text-red-500">*</span></label>
                                <select value={data.supplier_id} onChange={(e) => setData('supplier_id', e.target.value)} required
                                    className={`${inpH} ${errors.supplier_id ? 'border-red-400' : ''}`}>
                    ...
                    <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                            ...
                            <button type="button" onClick={addRow}
                                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm border border-blue-200 text-blue-600 rounded-lg hover:bg-blue-50 transition-colors">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Link href="/purchase-orders" className="inline-flex items-center px-4 py-2 text-sm text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">Batal</Link>
                    </div>
                </div>
            </form>
        </AppLayout>
